...
export default function DisplayAllCompleteGames() {
  ...
  function addtoDB_RequestedGames(data) {
    addFavGamesToDB(data);
  }

  function addFavGamesToDB(data) {
    let fav = resp[0].favoritos;
    const found = fav.find((x) => {
    ...
    if (found) {
      ...
      let newarr = [...fav, data];
      addJogosFavoritos(newarr).then(() => {});
    }
    setSelectedGameModal(false);
    getUserInfo();
  }
  function excludeFavFromDB(data) {
    let fav = resp[0].favoritos;
    const found = fav.find((x) => {
      return x.name === data.name;
    });
    if (found) {
      const filtered_arr = fav.filter((x) => {
        return x.name !== data.name;
      });
      let newarr = [...filtered_arr];
    ...
    <>
      ...
      <div className="container-all-coletion-general">
        ...
        <div className="container-all-coletion">
          ...
          {resp === null
            ? ""
            : resp[0].favoritos.map((fav) => {
                return (
                  <div key={fav.name} className="exp-2 " onClick={() => {}}>
                    {anotherUserProfile ? (
                      ""
                    ) : (
                      <GoX
